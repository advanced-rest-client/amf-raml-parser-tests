...
class AmfJsParserTestRunner {
  ...
  _forkParser(file) {
    return new Promise((resolve, reject) => {
      // https://github.com/nodejs/node/issues/9435#issuecomment-380458664
      const options = {
        execArgv: []
      };
      const proc = fork(`${__dirname}/${file}`, options);
  ...
  parseRamlParser() {
    ...
    .then((data) => {
      ...
      // fs.writeFileSync('raml-out.json', JSON.stringify(data.api, null, 2), 'utf8');
    });
  }
  /**
   * @return {Promise}
  ...
  _typesSizeTest() {
    ...
    if (this.ramlApi.schemas) {
      // AMF includes schemas into types.
      ramlTypesSize += this.ramlApi.schemas.length;
    }
  ...
  _testEndpointMethodsSize() {
    ...
    for (let i = 0; i < endpoints.length; i++) {
      ...
      let operations = this.__getAmfMethodForPath(path);
  ...
  _compareMethods(path, operation, method) {
    ...
    const securityReturns = this._computeSecurityReturns(
      operation[ns.raml.vocabularies.security + 'security']);
    if (returns && returns[0]) {
      // debugger;
      this._compareMethodsResponses(path, methodName, returns,
        method.responses, securityReturns);
    }
  }

  _computeSecurityReturns(model) {
    ...
    const ns = amfHelper.ns;
    model.forEach((item) => {
      const scheme = item[ns.raml.vocabularies.security + 'scheme'];
      if (!scheme || !scheme[0]) {
        return;
      }
      const responses = scheme[0][ns.w3.hydra.core + 'response'];
   ...
   * @param {?Array<Object>} securityResponses
   */
  _compareMethodsResponses(path, method, returns, responses,
    ...
    if (securityResponses && securityResponses.length) {
      sum = sum.concat(securityResponses);
    }
    if (!sum.length) {
      return;
    }
    // Resolved AMF can have more responses defined compared to raml2obj
    // because traits and resource types are applied to the output.
    // This test tests if a response defined in RAML is available in AMF
    // and ignores the case when AMF has more responses.
    if (!responses || !responses.length) {
      return;
    }
    const ns = amfHelper.ns;
    ...
    for (let i = 0, len = responses.length; i < len; i++) {
      const code = Number(responses[i].code);
      let hasStatus = false;
      for (let k = 0, kLen = sum.length; k < kLen; k++) {
        const amfReturn = amfHelper.resolve(this.amfApi, sum[k]);
      ...
      if (!hasStatus) {
        ...
        msg += `${code} response not defined in AMF model`;
        this.reportError(new Error(msg));
        errored = true;
      }
    }
